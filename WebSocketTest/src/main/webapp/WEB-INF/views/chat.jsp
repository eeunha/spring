	...
	<script>
		...
		*/
		...
		function connect(name) {
			window.name = name;
			$('#header small').text(name); //jQuery를 사용하여 HTML 문서에서 #header 아래에 있는 <small> 태그의 텍스트를 name으로 설정
			...
			ws.onopen = function(evt) {
				...
				ws.send('2|1|2|점심 뭐먹어?'); //상태코드|보내는사람|받는사람|대화
				ws.send('3|홍길동'); //상태코드|대화방을나간사람 */
			...
			ws.onmessage = function(evt) {
				...
				//console.log(evt.data); //문자열
				
				let message = JSON.parse(evt.data);
				//console.log(message); //JSON 객체
